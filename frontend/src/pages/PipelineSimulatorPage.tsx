  ...
  useEffect(() => {
    ...
    const drawStatic = () => {
      ...
      const seg = (x1: number, y1: number, x2: number, y2: number) =>
        el("line", { x1, y1, x2, y2, stroke: C.gold, "stroke-width": 2.5, "marker-end": "url(#plsim-ar)" });
    ...
    const drawListenerGroup = (g: SVGGElement) => {
      // Outer container for listener_mp.py
      const lx = 196, ly = 160, lw = 150, lh = 330;
      ...
      const fx = lx + 15, fy = ly + 30, fw = 120, fh = 36;
      ...
      // worker sub-boxes stacked vertically
      const wLabels = ["w-0", "w-1", "w-2", "w-3"];
      const wwBoxW = 60, wwBoxH = 30;
      const wx = lx + lw / 2 - wwBoxW / 2;
      const startY = qy2 + 15;
      const yStep = 45;
      
      wLabels.forEach((label, i) => {
        const wwY = startY + i * yStep;
        g.appendChild(box(wx, wwY, wwBoxW, wwBoxH, fillFor(C.blue), C.blue, 5));
        g.appendChild(txt(wx + wwBoxW / 2, wwY + 15, label, C.text, 10));
        
        // vertical fan line from queue bottom to each worker
        g.appendChild(el("line", { x1: qx, y1: qy2, x2: wx + wwBoxW / 2, y2: wwY, stroke: C.blue, "stroke-width": 1, opacity: 0.5 }));
        ...
        g.appendChild(el("line", {
          ...
          x2: 430, y2: kPartY,
          stroke: C.gold, "stroke-width": 2.5, "marker-end": "url(#plsim-ar)",
        }));
      });
    };
    ...
    const drawNodes = () => {
      ...
      assign.forEach((wk, p) => {
        ...
        g.appendChild(el("line", { x1: 564, y1: LANES[p], x2: 678, y2: workers[wk].lane, stroke: C.gold, "stroke-width": 2.5, "marker-end": "url(#plsim-ar)" }));
      });

      // worker -> mongo
      workers.forEach((w) => {
        if (!w.alive) return;
        g.appendChild(el("line", { x1: 860, y1: w.lane, x2: 968, y2: 275, stroke: C.gold, "stroke-width": 2.5, "marker-end": "url(#plsim-ar)" }));
      });
    ...
    const waypoints = (p: Packet) => {
      ...
      const wwY = 261 + p.partition * 45;
      const listenerWorkerY = wwY + 15;

      return [
        { x: 150, y: 335 },   // mock node edge
        { x: 195, y: 335 },   // into listener
        { x: 271, y: listenerWorkerY }, // inside listener to worker
        { x: 301, y: listenerWorkerY }, // listener worker right edge
        { x: 430, y: Li },    // kafka left edge
        { x: 564, y: Li },    // kafka right edge
        { x: 678, y: Lj },    // spark worker left edge
        { x: 860, y: Lj },    // spark worker right edge
        { x: 968, y: 275 },   // mongo left edge
        { x: 1055, y: 275 },  // mongo center
      ];
    };
    const spawn = () => {
