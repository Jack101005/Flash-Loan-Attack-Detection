...
function ConcurrencyVisual() {
  ...
  useEffect(() => {
    ...
    const LANES = [104, 154, 204, 254];

    // Mock node
    svg.appendChild(box(10, 154, 80, 50, fillFor(BLUE), BLUE));
    svg.appendChild(txt(50, 179, "Mock node", TEXT, 10));

    // listener_mp.py (outer)
    svg.appendChild(box(110, 40, 130, 240, fillFor(BLUE), BLUE, 10));
    svg.appendChild(txt(175, 54, "listener_mp.py", BLUE, 9));
    svg.appendChild(box(125, 64, 100, 20, fillFor(BLUE), BLUE, 5));
    svg.appendChild(txt(175, 74, "feeder", TEXT, 10));
    LANES.forEach((cy, i) => {
      svg.appendChild(box(125, cy - 12, 100, 24, fillFor(BLUE), BLUE, 4));
      svg.appendChild(txt(175, cy, `worker-${i}`, TEXT, 9));
    });
    svg.appendChild(el("line", { x1: 90, y1: 179, x2: 110, y2: 179, stroke: GOLD, "stroke-width": 2.5, "marker-end": "url(#demo-ar)" }));
    ...
    LANES.forEach((cy, i) => {
      svg.appendChild(box(270, cy - 14, 90, 28, fillFor(AMBER), AMBER, 5));
      svg.appendChild(txt(315, cy, `Partition ${i}`, TEXT, 9));
      svg.appendChild(el("line", { x1: 225, y1: cy, x2: 270, y2: cy, stroke: GOLD, "stroke-width": 2.5, "marker-end": "url(#demo-ar)" }));
    });

    // Spark master
    svg.appendChild(box(400, 40, 130, 34, fillFor(PURPLE), PURPLE, 8));
    ...
    const path = el("path", { d: "M2 1L8 5L2 9", fill: "none", stroke: GOLD, "stroke-width": "1.4" });
    m.appendChild(path);
    defs.appendChild(m);
    svg.insertBefore(defs, svg.firstChild);

    // Add animated red dots moving exactly along the arrows
    const addDot = (x1: number, y1: number, x2: number, y2: number, delay: string) => {
      const c = el("circle", { r: 4, fill: RED });
  ...
  return (
    <div className="px-6 py-4">
      <svg ref={svgRef} viewBox="0 0 700 310" className="w-full h-auto" style={{ maxHeight: 450 }} />
    ...
    <div className="px-6 py-4 space-y-4">
      <div className="relative overflow-x-auto">
        <svg viewBox="0 50 700 120" className="w-full" style={{ minWidth: 480, height: 120 }}>
          ...
          {dotT > 0 && (
            <g>
              <circle cx={dotX} cy={80} r={9} fill={RED} opacity={0.18} />
              <circle cx={dotX} cy={80} r={5} fill={RED} />
            </g>
          )}
        </svg>
      </div>
  ...
  return (
    <div className="px-6 py-4 space-y-5">
      ...
      <div className="rounded-[12px] p-4" style={{ background: BG, border: `1px solid ${LINE}` }}>
        ...
        <svg viewBox="0 0 720 320" className="w-full h-auto" style={{ maxHeight: 450 }}>
          <defs>
            <marker id="ft-ar" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse">
              <path d="M2 1L8 5L2 9" fill="none" stroke={GOLD} strokeWidth="1.4" />
            </marker>
          ...
          {workers.map((w, i) => {
            ...
            return (
              <g key={w.id} style={{ cursor: "pointer" }} onClick={() => toggleWorker(w.id)}>
                ...
                {/* packet animation: red dot moving along the partition-to-worker path */}
                {!dead && ownedParts.map((p, idx) => (
                  <circle key={`dot-${p}`} r="4" fill={RED} filter="url(#ft-glow)">
                    <animateMotion
                      dur="1s"
                      repeatCount="indefinite"
                      path={`M 130 ${108 + p * 48} L 258 ${wy + 18}`}
                      begin={`${idx * 0.3}s`}
                    />
                  </circle>
                ))}

                {/* worker -> mongo packet animation */}
                {!dead && (
                  <circle r="4" fill={RED} filter="url(#ft-glow)">
                    <animateMotion
                      dur="1s"
                      repeatCount="indefinite"
